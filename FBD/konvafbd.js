 ...
 Konva.Vector = class myvect extends Konva.Group {
    constructor(x, y, fixedLength) {  // fixedLength not implemented
 ...
 Konva.CurveLine = class mycurve extends Konva.Group {
    ...
    isDirectionTowards(point){
       // is the first point closer to the point than the last point
       ...
       renderCallback: function (dom, ansShapes) {
          ...
          ansShapes.forEach(item => {
             if (item.type.includes("graph")) {
                ...
                graphrect.on("xyupdate mousemove", (evt) => {
                ...
                let solnrect = new Konva.Line({
                   ...
                   opacity: 0.3,
                   strokeWidth: (item.type.includes("curve") ? 10 : 1),
                   name: item.type.trim(),
                   listening: false
                });
 ...
 function answerOverlap(answer, solution, tolerance, countOutside) {
    ...
    // there must be a built in function to get this bounding box, the code below has some error/roundoff
    ...
    var imageData = layerCalcs.getContext().getImageData(0, 0, 500, 500).data;  // grab everything


    var sum = 0;
    for (var i = 3; i < imageData.length; i += 4) {
       sum += (imageData[i] > 10 ? 1 : 0);  // 10 = lower bound for opacity
    }
 ...
 document.getElementById("checkSolution").addEventListener("click", (evt) => {
    ...
    solns.forEach((sol, idx) => {
       if (!isSatisfied[idx]) {
          ...
          // this redundantly rechecks answers that already satisfy a solution
          answ.forEach(ans => {
...
// Startup
 ...
 var graphs = [];
 document.getElementById("svgParams").value = SVGPARAMS;
 loadModel();
