		...
		function build(opts) {
			...
			svg.Element.ElementBase = function (node) {
				...
				if (node != null && node.nodeType == 1) { //ELEMENT_NODE
					...
					var jsonParamText = ""
					var jsonParamMap = {}
					if (node.attributes['params'] != undefined) {
						jsonParamText = node.attributes['params'].value
						for (let p in opts.params) {
							jsonParamText = jsonParamText.replaceAll(p, opts.params[p])
						}
						//jsonParamText=jsonParamText.replaceAll("$A","105")
						jsonParamMap = JSON.parse(jsonParamText)
					}
					// END FORK
					for (var i = 0; i < node.attributes.length; i++) {
						...
						// FORK
						if (jsonParamMap[nodeName] != undefined) {
							this.attributes[nodeName] = new svg.Property(nodeName, jsonParamMap[nodeName]);
						} else {
							// END FORK
			...
			svg.Element.rect = function (node) {
				...
				this.path = function (ctx) {
					...
					// FORK
					// capture the solution's global coordinates always
					if (this.attribute("class").hasValue() && this.attribute('class').value == "answer") {

						// must be a better way to do this, but need the transformed coords
						let xy = [new DOMPoint(x, y).matrixTransform(ctx.getTransform()),
						new DOMPoint(x + width, y).matrixTransform(ctx.getTransform()),
						new DOMPoint(x + width, y + height).matrixTransform(ctx.getTransform()),
						new DOMPoint(x, y + height).matrixTransform(ctx.getTransform())];

						let rectnodes = [xy[0].x, xy[0].y, xy[1].x, xy[1].y, xy[2].x, xy[2].y, xy[3].x, xy[3].y]

						svg.rtnparams.push({xy:rectnodes,opts:this.attribute('ansOpts').value + " closed",
						type:this.attribute('ansType').value, desc:this.attribute('ansDesc').value})
						
					} else if (this.attribute("class").hasValue() && this.attribute('class').value == "graph"){
						// assumes graphs are not rotated, so don't need to transform the height and width
						let xy = new DOMPoint(x, y).matrixTransform(ctx.getTransform());

						let rectnodes = [xy.x, xy.y, width, height]

						svg.rtnparams.push({xy:rectnodes,opts:this.attribute('ansOpts').value,
						type:this.attribute('ansType').value, desc:this.attribute('ansDesc').value})

					} else {

					// don't show the rectangle if the item is an answer
					// if ((this.attribute("class").hasValue() && this.attribute('class').value != "answer") || opts.showAns) {
						...
						switch (pp.command) {
							...
							case 'm':
								...
								if (this.attribute("class").hasValue() && this.attribute('class').value == "answer") {
									...
									xy = new DOMPoint(p.x, p.y).matrixTransform(ctx.getTransform())
									rectnodes = [xy.x, xy.y]
									while (!pp.isCommandOrEnd()) {
										p = pp.getAsCurrentPoint();

										xy = new DOMPoint(p.x, p.y).matrixTransform(ctx.getTransform())
										rectnodes.push(xy.x, xy.y)
									}
									svg.rtnparams.push({xy:rectnodes,opts:this.attribute('ansOpts').value,
									     type:this.attribute('ansType').value, desc:this.attribute('ansDesc').value })
									//console.log(solnOrigin.x, solnOrigin.y, solnEnd.x, solnEnd.y);
								} else {

								// don't show the rectangle if the item is an answer area
								//if ((this.attribute("class").hasValue() && this.attribute('class').value != "answer") || opts.showAns) {
			...
			svg.Element.tspan = function (node) {
				...
				this.getText = function () {
					...
					// FORK
					for (let p in opts.params) {
						this.text = this.text.replaceAll(p, opts.params[p])
					}
					// END FORK
					return this.text;
				};
			...
			svg.Element.g = function (node) {
				// FORK
				// don't render the node if is a answer node
				if (this.attribute("class").hasValue() && this.attribute('class').value == "answer") {
				//if (node.getAttribute("class") == "solution" && !opts.showAns) {
			...
			svg.loadXmlDoc = function (ctx, dom) {
				...
				var draw = function () {
					...
					if (isFirstRender) {
						isFirstRender = false;
						if (typeof svg.opts['renderCallback'] == 'function') svg.opts['renderCallback'](dom,svg.rtnparams);
					}
				};
